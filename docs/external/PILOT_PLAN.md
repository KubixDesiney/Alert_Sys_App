...
## 1. Objective
Prove SIA measurably shortens alert response time and improves coordination on one
...
- Integration: feed SIA from the line's existing alarms via the ingestion connector
...
|---|---|---|---|---|
| Mean time to acknowledge (alert → claimed) | SIA alert timestamps vs current radio/log baseline | `[__]` | ≥ 30% faster | alert lifecycle fields |
...
- **Weeks 1–N (live):** SIA in use; the same metrics auto-collected from alert records.
