# Competitive Positioning — SIA vs SCADA / IIoT

An honest map of where SIA wins, where it complements, and where it deliberately
...
**SIA is the mobile alerting, dispatch, and predictive-intelligence layer for the
factory floor — it makes an existing SCADA/IIoT estate act on its own data faster.**

## The category, precisely
SIA is **not** a SCADA/DCS (no control loops, no PLC programming, no hard-real-time
...
## Where SIA wins (vs the alerting features bundled into SCADA/IIoT suites)

| Capability | Typical SCADA/IIoT alarm module | SIA |
|---|---|---|
...
## Where SIA complements (and must integrate, not fight)

| Incumbent | Relationship |
|---|---|
| **SCADA/DCS** (Ignition, Wonderware, FactoryTalk, WinCC) | They own control + visualization; SIA ingests their alarms/tags (`SCADA_INTEGRATION.md`) and runs the human response. |
| **Historian** (PI, Ignition Historian) | Source of telemetry/backfill for the forecaster; SIA does not store time-series at historian scale. |
| **CMMS/EAM** (Maximo, Fiix, UpKeep) | SIA handles the real-time alert→claim→resolve loop; a work order can be opened from a resolved alert. SIA is faster at the live incident, weaker at long-horizon asset lifecycle. |
| **MES/quality** | POST quality excursions to SIA as Quality alerts. |

## Where SIA does **not** compete (say so plainly)
- Closed-loop process **control** / setpoint management — stays in PLC/DCS.
- Safety-instrumented systems (SIS), functional-safety SIL loops — SIA is not a
...
- **Ceiling:** as a category, SIA competes as a *complement*, so the dimension tops out below a hypothetical "replaces SCADA" 10 — that 10 isn't a product SIA is trying to be. The realistic max is "the default alerting/dispatch layer bolted onto any SCADA estate," which the integration connector + positioning now support.

## Objection handling (quick)
- *"We already have SCADA alarms."* → Those alarm the control room; SIA gets the
...
- *"Is this another rip-and-replace?"* → No. SIA reads from your stack and changes
