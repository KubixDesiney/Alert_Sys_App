...
async function collectWorkerHealth(signals) {
  ...
  if (workerSecret) headers['X-SIAS-Worker-Secret'] = workerSecret;
...
async function gatherContext(signals, feedback, validation, review) {
  ...
  return {
    generatedAt: new Date().toISOString(),
    mission:
      'Autonomously fix this SIAS repo. Preserve product architecture in CLAUDE.md. Make the smallest safe code change.',
    constraints: [
...
async function createAutofixPullRequest({ branch, title, changedPaths, signals, validation, review }) {
  ...
  const body = [
    'Automated bug-fix proposal generated by the SIAS autonomous agent.',
    '',
