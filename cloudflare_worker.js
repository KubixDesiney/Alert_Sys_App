// Cloudflare Worker – AlertSys (Instant AI, no spam, stable, FULL)
// Cron schedule: "* * * * *" (every minute – escalation / predictions / briefing only)
// Required env vars: WORKER_SHARED_SECRET, FB_DB_URL, FB_API_KEY, FIREBASE_SERVICE_ACCOUNT
// Optional: enable Workers AI binding for Llama 3.2 3B

const corsHeaders = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Methods': 'GET, POST, OPTIONS',
  'Access-Control-Allow-Headers': 'Content-Type, X-AlertSys-Worker-Secret, X-Firebase-ID-Token',
};
...
const MAX_ALERTS_TO_PUSH = 1;           // maximum alerts to push per broadcast run
const MAX_ESCALATION_CHECKS = 5;        // maximum escalations checked per cron tick
const MAX_FANOUT = 2;                   // maximum notification fan-outs per /notify call
const MAX_AI_FACTORIES = 20;            // factories scanned per AI pass
const MAX_AI_ASSIGNMENTS_PER_TICK = 1;  // only one assignment per trigger (HTTP or cron)

function buildActionId(prefix, ...parts) {
  return [prefix, ...parts]
...
// ============================================================
// Firebase ID token verification (admin-only endpoints)
// ============================================================
async function verifyFirebaseIdToken(idToken, env) {
  if (!idToken) throw new Error('Missing ID token');
  const url = `https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${env.FB_API_KEY}`;
  const res = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const data = await res.json();
  if (!data?.users?.[0]?.localId) throw new Error('Invalid token');
  const user = data.users[0];
  const customClaims = user.customAttributes ? JSON.parse(user.customAttributes) : {};
  if (customClaims.role !== 'admin' && user.role !== 'admin') {
...
function getFcmTokensForFactory(factoryName, usersMap, alertsMap) {
  ...
  for (const [uid, user] of Object.entries(usersMap)) {
    ...
    } else if (user.role !== 'admin') continue;
...
// ============================================================
// New‑alert FCM push (broadcast for alerts NOT auto‑assigned)
// ============================================================
  ...
  for (const alert of unsent) {
    ...
    for (const tok of fcmTokens) {
      const ok = await sendFcm(
        ...
        { alertId: alert.id, type: alert.type, usine: alert.usine, voiceAction: 'true' },
        env,
      );
      if (!ok) allOk = false;
    }
...
async function checkEscalations(env, ctx) {
  ...
  for (const [alertId, alert] of Object.entries(alertsMap)) {
    ...
    try {
      ...
      const actionId = buildActionId('escalate', alertId);
      ...
      if (!patchRes.ok) { console.error(`[ESCALATION] Failed to patch alert ${alertId}: ${patchRes.status}`); processed++; continue; }
...
function buildPredictiveModel(alertsMap) {
  ...
  const hodCounts = {}, dowCounts = {}, recentCounts = {}, machineHistory = {}, factoryRisk = {};
  for (const t of PREDICT_TYPES) { hodCounts[t] = new Array(24).fill(0); dowCounts[t] = new Array(7).fill(0); recentCounts[t] = 0; }
  for (const [, a] of Object.entries(alertsMap || {})) {
    ...
    const hod = d.getUTCHours(), dow = d.getUTCDay();
    hodCounts[type][hod]++; dowCounts[type][dow]++; recentCounts[type]++;
    const factoryId = aiSanitizeFactoryId(a.usine || '');
    const conv = a.convoyeur ?? 0, post = a.poste ?? 0;
    const mKey = `${factoryId}|${conv}|${post}|${type}`;
    const ageDays = (now - tsMs) / 86400000;
    const decay = Math.exp(-ageDays / PREDICT_HALFLIFE_DAYS);
    if (!machineHistory[mKey]) machineHistory[mKey] = { factoryId, usine: a.usine || '', convoyeur: conv, poste: post, type, score: 0, count: 0, lastTs: tsMs, firstTs: tsMs, critical: 0 };
    const m = machineHistory[mKey]; m.score += decay; m.count++;
    if (a.isCritical) m.critical++;
    if (tsMs > m.lastTs) m.lastTs = tsMs; if (tsMs < m.firstTs) m.firstTs = tsMs;
    if (!factoryRisk[factoryId]) factoryRisk[factoryId] = { name: a.usine || factoryId, score: 0, count: 0 };
    factoryRisk[factoryId].score += decay; factoryRisk[factoryId].count++;
  }
  ...
  for (const type of PREDICT_TYPES) {
    ...
    for (let i = 0; i < 12; i++) {
      const h1 = (startHour + i * 2) % 24, h2 = (startHour + i * 2 + 1) % 24;
      ...
      total += probability;
      buckets.push({ offsetHours: i * 2, startHour: h1, endHour: h2, probability: Number(probability.toFixed(4)), expected: Number(lambda.toFixed(4)) });
    }
    const totalRecent = recentCounts[type], dailyAvg = totalRecent / PREDICT_HORIZON_DAYS;
    const peak = buckets.reduce((p, c) => (c.probability > p.probability ? c : p), buckets[0]);
    curves[type] = { buckets, total24h: Number((1 - Math.exp(-dailyAvg)).toFixed(4)), hourlyRate: Number((dailyAvg / 24).toFixed(4)), peakHour: peak.startHour, peakProbability: peak.probability, avgProbability: Number((total / 12).toFixed(4)), sampleSize: totalRecent };
  }

  const ranked = Object.values(machineHistory).filter(m => m.count >= 1).sort((a,b) => b.score - a.score).slice(0,10);
  ...
  const predictions = ranked.map(m => {
    ...
    return { factoryId: m.factoryId, usine: m.usine, convoyeur: m.convoyeur, poste: m.poste, type: m.type, confidence, pastCount: m.count, criticalCount: m.critical, lastTs: new Date(m.lastTs).toISOString(), etaHours: etaHours !== null ? Number(etaHours.toFixed(1)) : null, score: Number(m.score.toFixed(3)) };
  });

  const factoryRanked = Object.entries(factoryRisk).map(([id, v]) => ({ id, name: v.name, score: Number(v.score.toFixed(3)), count: v.count })).sort((a,b) => b.score - a.score).slice(0,6);
  return { curves, predictions, factoryRisk: factoryRanked, generatedAt: new Date().toISOString(), horizonDays: PREDICT_HORIZON_DAYS, halflifeDays: PREDICT_HALFLIFE_DAYS };
}
...
async function refreshPredictionsIfStale(env, ctx, maxAgeMin = 30) {
  ...
  await fetch(`${env.FB_DB_URL}ai_predictions/latest.json?auth=${token}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(model) });
  return model;
}

// ============================================================
// Morning Briefing (Workers AI Llama 3.2 3B)
// ============================================================
...
function _aggregateWeek(alertsMap) {
  ...
  const stats = { total: 0, solved: 0, pending: 0, inProgress: 0, critical: 0, aiAssigned: 0, byType: {}, byFactory: {}, avgResolutionMin: 0, fastestMin: null, slowestMin: null };
  let totalElapsed = 0, solvedCount = 0;
  for (const a of Object.values(alertsMap || {})) {
    if (!a) continue;
    const tsMs = _toMs(a.timestamp);
    if (!tsMs || (now - tsMs) > WEEK) continue;
    stats.total++;
    if (a.status === 'validee') { stats.solved++; if (typeof a.elapsedTime === 'number' && a.elapsedTime > 0) { totalElapsed += a.elapsedTime; solvedCount++; if (stats.fastestMin === null || a.elapsedTime < stats.fastestMin) stats.fastestMin = a.elapsedTime; if (stats.slowestMin === null || a.elapsedTime > stats.slowestMin) stats.slowestMin = a.elapsedTime; } }
    else if (a.status === 'en_cours') stats.inProgress++;
    else stats.pending++;
    if (a.isCritical) stats.critical++;
    if (a.aiAssigned) stats.aiAssigned++;
    const t = String(a.type || 'other'); stats.byType[t] = (stats.byType[t] || 0) + 1;
    const f = String(a.usine || 'unknown'); stats.byFactory[f] = (stats.byFactory[f] || 0) + 1;
  }
...
function _typeName(t) {
  return ({ qualite: 'Quality', maintenance: 'Maintenance', defaut_produit: 'Damaged Product', manque_ressource: 'Resource Deficiency' })[t] || t;
}
...
async function generateMorningBriefing(env, ctx, force = false) {
  ...
  if (!force) {
    try {
      ...
      if (cur.ok) { const data = await cur.json(); if (data?.date === today) return data; }
    } catch (e) {}
  }
...
Begin with "Good morning". Acknowledge what is going well, name one specific area to watch, and close with a forward-looking sentence about today. Sound calm, professional, and human — not a press release.`;
  ...
  try {
    if (env.AI) {
      const resp = await env.AI.run('@cf/meta/llama-3.2-3b-instruct', { messages: [{ role: 'user', content: prompt }] });
      const out = (resp.response || '').trim();
      if (out) { summary = out; model = '@cf/meta/llama-3.2-3b-instruct'; }
    }
  } catch (e) { console.error('[BRIEFING] AI failed: ' + e.message); }

  const payload = { date: today, summary, generatedAt: new Date().toISOString(), model, stats, topType: topType ? { type: topType[0], count: topType[1] } : null, topFactory: topFactory ? { name: topFactory[0], count: topFactory[1] } : null, resolutionRate };
  await fetch(`${env.FB_DB_URL}ai_briefing/latest.json?auth=${token}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  await fetch(`${env.FB_DB_URL}ai_briefing/history/${today}.json?auth=${token}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
  return payload;
}

// ============================================================
// AI proxy (Llama 3.2 3B via Workers AI)
// ============================================================
async function handleAIRequest(request, env) {
  ...
  try {
    ...
    if (!env.AI) {
      return new Response(JSON.stringify({ suggestion: fallback, note: 'AI binding not configured' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    const response = await env.AI.run('@cf/meta/llama-3.2-3b-instruct', { messages: [{ role: 'user', content: prompt }] });
    ...
    return new Response(JSON.stringify({ suggestion: fallback, note: String(e) }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
}
...
async function handleAutoFix(request, env) {
  try {
    ...
    if (!env.AI) return new Response(JSON.stringify({ fixedCode: '', note: 'AI binding not configured' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    const prompt = [
    ...
    const response = await env.AI.run('@cf/meta/llama-3.2-3b-instruct', { messages: [{ role: 'user', content: prompt }], max_tokens: 4096 });
    const fixedCode = (response.response ?? '').trim();
    return new Response(JSON.stringify({ fixedCode }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
    return new Response(JSON.stringify({ fixedCode: '', note: String(e) }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
}

async function handleAutoFixFull(request, env) {
  try {
    const { files, failingTest, errors } = await request.json();
    if (!env.AI) return new Response(JSON.stringify({ fixedFiles: [], note: 'AI binding not configured' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    const fileList = files.map(f => `=== ${f.path} ===\n${f.content}`).join('\n\n');
    const prompt = [
      'You are an elite Dart/Flutter and Cloudflare Worker engineer. You have access to an entire project\'s source code.',
      'A test is failing. Analyze the code and the error output to determine whether the test itself is incorrect',
      '(e.g. a wrong expected value that does not match the production code) or the production code contains a bug.',
      'Edit ONLY the files that need to change (either the test, the production code, or both). Do NOT edit files that are correct.',
      'Return ONLY a JSON array of objects, each with "path" and "content" for every file you modified.',
    ...
    try { fixedFiles = JSON.parse(raw); } catch { const match = raw.match(/\[[\s\S]*\]/); if (match) { try { fixedFiles = JSON.parse(match[0]); } catch {} } }
    return new Response(JSON.stringify({ fixedFiles }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
    return new Response(JSON.stringify({ fixedFiles: [], note: String(e) }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
}

// ============================================================
// Fan‑out notifications (unchanged)
// ============================================================
...
// ============================================================
// AI Assignment engine (UPDATED: push_sent true on assignment)
// ============================================================
...
async function aiAssignAlert(alertId, supervisor, reasonSummary, confidence, env, token) {
  ...
  const actionId = buildActionId('ai_assign', alertId, supervisor.uid);
  ...
  const putRes = await fetch(alertUrl, {
    ...
    body: JSON.stringify({
      ...
      aiAssignedAt: nowIso,
      push_sent: true,                      // ← CRITICAL: prevents broadcast
    }),
  });
  ...
  if (supervisor.fcmToken) {
    await sendFcm(
      ...
      { type: 'ai_assigned', actionId, alertId: String(alertId), recipientId: String(supervisor.uid), reason: reasonSummary },
      env,
    );
  }
  return true;
...
async function runAIAssignments(env, ctx) {
  ...
  if (ctx) { alertsMap = ctx.alertsMap; usersMap = ctx.usersMap; }
  else {
    const [ar, ur] = await Promise.all([ fetch(`${env.FB_DB_URL}alerts.json?auth=${token}`), fetch(`${env.FB_DB_URL}users.json?auth=${token}`) ]);
    if (!ar.ok || !ur.ok) return;
    alertsMap = (await ar.json()) || {}; usersMap = (await ur.json()) || {};
  }
  ...
  for (let i = 0; i < Math.min(factoryIds.length, MAX_AI_FACTORIES); i++) {
    if (assignedCount >= MAX_AI_ASSIGNMENTS_PER_TICK) break;
...
// ============================================================
// /suggest-assignee (one-tap suggestion)
// ============================================================
async function handleSuggestAssignee(request, env) {
  try {
    ...
    if (!alertId) {
      return new Response(JSON.stringify({ error: 'alertId required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!alert) {
      return new Response(JSON.stringify({ error: 'alert not found' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
...
// ============================================================
// /admin/create-supervisor (secured: shared secret + Firebase ID token)
// ============================================================
async function handleCreateSupervisor(request, env) {
  try {
    const { firstName, lastName, email, password, phone, usine, hiredDate } = await request.json();
    ...
    const createUrl = `https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${env.FB_API_KEY}`;
    ...
    const token = await getFirebaseToken(env);
    await fetch(`${env.FB_DB_URL}users/${uid}.json?auth=${token}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        email,
    ...
    return new Response(JSON.stringify({ uid }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
    const msg = e.message || 'Unknown error';
    return new Response(JSON.stringify({ error: msg }), {
      status: msg === 'Missing admin token' ? 401 : 400,
...
function handleConfigRequest() {
  return new Response(JSON.stringify({ message: 'Config endpoint deprecated' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
}

// ============================================================
// Main fetch + scheduled export
// ============================================================
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/ai-retry') {
      ...
      return new Response(JSON.stringify({ queued: true }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    // Default (manual trigger): AI first, then broadcast
    try {
      const coreCtx = await loadCoreData(env);
      await runAIAssignments(env, coreCtx);
      await processAlerts(env, coreCtx);
    } catch (e) { console.error('[MANUAL] Error: ' + e.message); }
