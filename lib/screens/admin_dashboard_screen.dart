...
import 'dart:convert';
import 'dart:html' as html;  // only for web download; for mobile you'd use share_plus or path_provider
...
// ── Palette (matching screenshot — white/navy/red) ─────────────────────────
        ...
        child: ListView.builder(
          ...
          itemBuilder: (_, i) => ListTile(
            ...
            onTap: () async {
              ...
              // Refresh the alert list (the stream will update automatically)
            },
          ),
        ),
      ),
...
Future<void> _createSimulatedAlert({
  ...
  // Optional: send push notification via Cloudflare Worker
  try {
    await http.post(
    ...
    switch (_tab) {
      case 0:
        return _OverviewTab(
          ...
          onExportCsv: _exportToCsv,      // ✅ add this
          onExportExcel: _exportToExcel,   // ✅ add this
          allAlerts: _alerts,
        );
      case 1:
...
// ═══════════════════════════════════════════════════════════════════════════
// HEADER — matches screenshot: logo + title + subtitle + logout button
// ═══════════════════════════════════════════════════════════════════════════
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        ...
        // Logout button — matches screenshot exactly
        OutlinedButton.icon(
...
// ═══════════════════════════════════════════════════════════════════════════
// PILL TAB BAR — matches screenshot (horizontal chip style)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// OVERVIEW TAB — with clickable history filter
// ═══════════════════════════════════════════════════════════════════════════
...
List<AlertModel> get _criticalUnclaimedAlerts {
  // Use allAlerts (unfiltered) so critical alerts are shown regardless of time/plant filters
  ...
  void _setHistoryFilter(String? filter) {
    setState(() {
      if (_historyFilter == filter) _historyFilter = null;
      else _historyFilter = filter;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // ── Critical Unclaimed Alerts Red Box (FORCED to show for testing) ──
        // Replace `true` with `_criticalUnclaimedCount > 0` once verified.
  ...
  void _exportFilteredAlerts(List<AlertModel> alertsToExport) {
  ...
  void _exportFilteredAlertsExcel(List<AlertModel> alertsToExport) {
    if (alertsToExport.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No alerts to export'), backgroundColor: Colors.orange),
      );
...
// ── Big Stat Card (top 3 in screenshot) ───────────────────────────────────
...
// ── Type Card (4 cards row in screenshot) ─────────────────────────────────
class _TypeCard extends StatelessWidget {
  final String type;
  final int    total, solved, pending;
  const _TypeCard({required this.type, required this.total,
      required this.solved, required this.pending});
...
// ── Alert history row (in the history section) ─────────────────────────────
...
// ═══════════════════════════════════════════════════════════════════════════
// SUPERVISORS TAB  — 3 sub-tabs: Management | Performance | Assignments
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// SUB-TAB 1 — MANAGEMENT (existing supervisor list, unchanged logic)
// ═══════════════════════════════════════════════════════════════════════════
...
// Select a supervisor → see fixed count, avg time, factory distribution,
// type breakdown cards, line chart of validations per day
// ═══════════════════════════════════════════════════════════════════════════
...
class _PerformanceSubTabState extends State<_PerformanceSubTab> {
  UserModel? _selected;
  String     _chartRange = '7days'; // '7days' | '30days'
 
  // ── Computed for selected supervisor ──────────────────────────────────────
  ...
  // Per-usine distribution of solved alerts
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (_selected != null) ...[
          // ── Top stat cards (Fixed + Avg time) ───────────────────────────
          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            // Fixed alerts card — matches screenshot style
            Expanded(
              flex: 2,
              child: Container(
                padding: const EdgeInsets.all(18),
...
// ── Small stat row inside type card ───────────────────────────────────────
...
// ── Line chart (CustomPainter — no external package needed) ───────────────
class _ChartPoint {
  final DateTime day;
  final double   value;
...
class _AssignmentsSubTab extends StatelessWidget {
  ...
  // City names (cosmetic, matches screenshot style)
...
// ── Supervisor chip for assignments tab ───────────────────────────────────
...
class _SupervisorCardState extends State<_SupervisorCard> {
  ...
  final firstNameCtrl = TextEditingController(text: sup.firstName);
  final lastNameCtrl = TextEditingController(text: sup.lastName);
  final phoneCtrl = TextEditingController(text: sup.phone);
  final usines = ['Usine A', 'Usine B', 'Usine C', 'Usine D'];
  String selectedUsine = sup.usine;
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  // Email (read‑only)
                  TextFormField(
                    initialValue: sup.email,
                    readOnly: true,
                    decoration: const InputDecoration(
                      labelText: 'Email (cannot be changed here)',
                      hintText: 'Use password reset to change email',
                    ),
                  ),
                  const SizedBox(height: 8),
                  // Plant dropdown
                  DropdownButtonFormField<String>(
                    value: selectedUsine,
                    items: usines.map((u) => DropdownMenuItem(value: u, child: Text(u))).toList(),
                    onChanged: (v) => setState(() => selectedUsine = v!),
                    decoration: const InputDecoration(labelText: 'Plant (Usine)'),
                  ),
                  const SizedBox(height: 8),
                  // Password (optional) – will send reset email
                  TextField(
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  final newLastName = lastNameCtrl.text.trim();
                  ...
                  // If password field is not empty, send password reset email
                  final newPassword = passwordCtrl.text.trim();
                  if (newPassword.isNotEmpty) {
                    // Since we cannot set another user's password directly from client,
                    // we send a password reset email to the supervisor's email.
                    await AuthService().sendPasswordResetEmail(sup.email);
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('Password reset email sent to supervisor. They will set a new password.'),
                  ...
                  if (context.mounted) {
                    Navigator.pop(context);
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('✅ ${newFirstName} ${newLastName} updated'), backgroundColor: _green),
                    );
                    // Refresh supervisor list
                    (context.findAncestorStateOfType<_AdminDashboardScreenState>())?._loadSupervisors();
                  }
                },
                child: isLoading
...
// ═══════════════════════════════════════════════════════════════════════════
// ALERTS TAB — all logic unchanged
// ═══════════════════════════════════════════════════════════════════════════
...
// ── Clickable Stat Card (with toggle hint) ─────────────────────────────────
      ...
      child: Container(
        ...
        child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Text('$value',
                style: TextStyle(fontSize: 34, fontWeight: FontWeight.w800,
                    color: color, height: 1)), // always use the card's color
            if (showHistoryHint)
              Padding(
...
// ── Clickable Type Card (with toggle hint) ─────────────────────────────────
