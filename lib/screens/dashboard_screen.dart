...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  int _bottomNavIndex = 0; // Always 0 since only Dashboard exists
  ...
  Widget build(BuildContext context) {
    ...
    final myInProgress = provider.inProgressAlerts(_superviseurId);
    final validated = provider.validatedAlerts(_superviseurId);
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            // Custom Bottom Navigation Bar - Only Dashboard Menu
            Container(
              decoration: const BoxDecoration(
                color: _white,
              ...
              child: Row(
                ...
                children: [
                  _BottomNavItem(
                    ...
                    label: 'Dashboard',
                    isSelected: true, // Always selected since only one item
                    onTap: () {
                      // Keep dashboard selected (no action needed)
                      setState(() {
                        _bottomNavIndex = 0;
                      });
                    },
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(icon, color: isSelected ? _white : const Color(0xFF64748B), size: 20),
            const SizedBox(height: 4),
            Text(
              label,
              style: TextStyle(
                ...
                color: isSelected ? _white : const Color(0xFF64748B),
              ),
            ),
          ],
        ),
            ...
            children: [
              ...
              OutlinedButton.icon(
                onPressed: () async {
                  ...
                  // Create PM action for the original supervisor
          ...
          OutlinedButton.icon(
            onPressed: () async {
              ...
              if (context.mounted) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('PM action marked as read'), backgroundColor: Colors.green),
                );
              }
            },
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Stack(clipBehavior: Clip.none, children: [
          IconButton(
            icon: const Icon(Icons.notifications_none, color: _navy, size: 28),
...
Future<void> _toggleCritical(AlertModel alert, BuildContext context) async {
  ...
  Future<void> _requestAssistance(BuildContext context, AlertModel alert) async {
    ...
    final pmRef = FirebaseDatabase.instance.ref('pm_actions/$currentUserId').push();
    await pmRef.set({
      'title': 'Assistance Requested',
      'description': 'You requested assistance on alert: ${alert.description}',
      'timestamp': DateTime.now().toIso8601String(),
      'status': 'unread',
      'alertId': alert.id,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Assistance request sent to all supervisors and manager.'),
          backgroundColor: Colors.orange),
    );
  }

  Future<void> _offerAssistance(BuildContext context, AlertModel alert) async {
    await provider.requestHelp(
      alert.id,
  ...
  String _getStatusLabel(AlertModel alert, bool isMine, String currentUserId) {
    final hasValidAssistant = alert.assistantId != null &&
        alert.assistantId != alert.superviseurId;
