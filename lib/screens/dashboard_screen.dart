...
class _OriginalDashboardContent extends StatefulWidget {
  ...
  @override
  State<_OriginalDashboardContent> createState() => _OriginalDashboardContentState();
}
...
class _HeaderState extends State<_Header> with SingleTickerProviderStateMixin {
                ...
                children: [
                          ...
                          children: [
                            Container(
                              ...
                              child: Text('$_notificationCount unread',
                                  style: const TextStyle(color: _white, fontWeight: FontWeight.bold, fontSize: 12)),
                            ),
                            const SizedBox(width: 8),
                            GestureDetector(
                  ...
                  Container(
                    ...
                    child: TabBar(
                      ...
                      indicator: BoxDecoration(color: _white, borderRadius: BorderRadius.circular(30), boxShadow: const [
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: Text('${_notifications.length}',
                                    style: const TextStyle(color: _white, fontSize: 10, fontWeight: FontWeight.bold)),
                              ),
                            ],
                          ),
                        ),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(n['message'] ?? 'Help request', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                    const SizedBox(height: 4),
                    Text(n['alertDescription'] ?? 'Action required', style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
                    const SizedBox(height: 8),
                    Row(
                      children: [
                        Icon(Icons.access_time, size: 14, color: Colors.grey.shade500),
                        const SizedBox(width: 4),
                        Text(_formatTimestamp(DateTime.parse(n['timestamp'] ?? DateTime.now().toIso8601String())),
                            style: TextStyle(color: Colors.grey.shade500, fontSize: 12)),
                        if (isBuzzingForThis) ...[
                          const SizedBox(width: 12),
                          const Icon(Icons.vibration, size: 14, color: Colors.red),
                          const SizedBox(width: 4),
              ...
              if (isUnread) Container(width: 10, height: 10, decoration: const BoxDecoration(color: _red, shape: BoxShape.circle)),
            ],
          ),
          const SizedBox(height: 16),
          Row(
            children: [
              OutlinedButton.icon(
                onPressed: () async {
                  await Provider.of<AlertProvider>(context, listen: false).acceptHelp(n['alertId'], n['helpRequestId']);
                  final alertSnapshot = await FirebaseDatabase.instance.ref('alerts/${n['alertId']}').once();
                  final alertData = alertSnapshot.snapshot.value as Map?;
                  final originalSupervisorId = alertData?['superviseurId'];
                  if (originalSupervisorId != null && originalSupervisorId != FirebaseAuth.instance.currentUser!.uid) {
                    final pmRef = FirebaseDatabase.instance.ref('pm_actions/$originalSupervisorId').push();
                    await pmRef.set({
                      'title': 'Assistant Assigned',
                      'description': '${FirebaseAuth.instance.currentUser?.displayName ?? 'A supervisor'} accepted your assistance request',
                  ...
                  if (context.mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Help request accepted'), backgroundColor: Colors.green));
                  }
                },
              ...
              OutlinedButton.icon(
                onPressed: () async {
                  await Provider.of<AlertProvider>(context, listen: false).refuseHelp(n['alertId'], n['helpRequestId']);
                  await _db.child('notifications/${FirebaseAuth.instance.currentUser!.uid}/${n['id']}').remove();
                  if (context.mounted) {
                    setModalState(() {
                      _notifications.removeWhere((item) => item['id'] == n['id']);
                    ...
                    if (_buzzingNotificationId == n['id']) _stopBuzzing();
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Help request refused'), backgroundColor: Colors.orange));
                  }
                },
              ...
              if (isUnread) ...[
                const SizedBox(width: 8),
                OutlinedButton.icon(
                  onPressed: () async { await _stopBuzzing(); setModalState(() {}); },
                  icon: const Icon(Icons.vibration, size: 16, color: Colors.red),
                  label: const Text('Stop Buzzing', style: TextStyle(color: Colors.red)),
                  style: OutlinedButton.styleFrom(side: const BorderSide(color: Colors.red), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
                ),
              ],
            ],
  ...
  Widget _buildAssistanceRequestItem(Map<String, dynamic> n, bool isUnread, StateSetter setModalState, BuildContext context) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      padding: const EdgeInsets.all(16),
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(n['message'] ?? 'Assistance request', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                    const SizedBox(height: 4),
                    Text(n['alertDescription'] ?? '', style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
                    const SizedBox(height: 8),
                    Row(
                      children: [
                        Icon(Icons.access_time, size: 14, color: Colors.grey.shade500),
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (supervisors.isEmpty) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('No active supervisors available')));
                return;
              }
              showDialog(
                context: context,
                  ...
                  content: SizedBox(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) => ListTile(
                        ...
                        onTap: () async {
                          Navigator.pop(_);
                          await AuthService().assignAssistantToAlert(n['alertId'], supervisors[i].id, supervisors[i].fullName);
                          await _db.child('notifications/${FirebaseAuth.instance.currentUser!.uid}/${n['id']}').remove();
                          if (context.mounted) {
                            setModalState(() {
                              _notifications.removeWhere((item) => item['id'] == n['id']);
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('✅ Assigned ${supervisors[i].fullName} as assistant'), backgroundColor: Colors.green));
                        },
                      ),
                    ),
                  ),
                  actions: [TextButton(onPressed: () => Navigator.pop(_), child: const Text('Cancel'))],
                ),
              );
            },
  ...
  Widget _buildDefaultNotificationItem(Map<String, dynamic> n, bool isUnread, StateSetter setModalState, BuildContext context) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: _white, border: Border.all(color: const Color(0xFFE5E7EB)), borderRadius: BorderRadius.circular(12)),
      child: ListTile(
        ...
        trailing: Row(
          ...
          children: [
            ...
            IconButton(
              ...
              onPressed: () async {
                if (isUnread) await _db.child('notifications/${FirebaseAuth.instance.currentUser!.uid}/${n['id']}').remove();
                if (context.mounted) {
                  Navigator.pop(context);
                  Navigator.push(context, MaterialPageRoute(builder: (_) => AlertDetailScreen(alertId: n['alertId'])));
                }
              },
            ),
          ],
  ...
  Widget _buildPmActionItem(Map<String, dynamic> action, bool isUnread, StateSetter setModalState, BuildContext context) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      padding: const EdgeInsets.all(16),
      ...
      child: Column(
        ...
        children: [
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Icon(Icons.assignment_turned_in, color: Colors.green, size: 24),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(action['title'] ?? 'PM Action', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                    const SizedBox(height: 4),
                    Text(action['description'] ?? '', style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
                    const SizedBox(height: 8),
                    Row(
                      children: [
                        Icon(Icons.access_time, size: 14, color: Colors.grey.shade500),
                        const SizedBox(width: 4),
                        Text(_formatTimestamp(DateTime.parse(action['timestamp'] ?? DateTime.now().toIso8601String())),
                            style: TextStyle(color: Colors.grey.shade500, fontSize: 12)),
                      ],
                    ),
                  ],
          ...
          OutlinedButton.icon(
            onPressed: () async {
              await _db.child('pm_actions/${FirebaseAuth.instance.currentUser!.uid}/${action['id']}').remove();
              if (context.mounted) {
                setModalState(() { _pmActions.removeWhere((item) => item['id'] == action['id']); });
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('PM action marked as read'), backgroundColor: Colors.green));
              }
            },
            icon: const Icon(Icons.check_circle_outline, size: 16, color: Colors.black87),
            label: const Text('Mark as read', style: TextStyle(color: Colors.black87)),
            style: OutlinedButton.styleFrom(backgroundColor: _white, side: BorderSide(color: Colors.grey.shade300), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
          ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: const BoxDecoration(color: _white, border: Border(bottom: BorderSide(color: Color(0xFFE5E7EB), width: 1))),
        ...
        Row(
          children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Supervisor', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _navy)),
              Text(widget.userName, style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
            ]),
          ],
        ),
        const Spacer(),
        Stack(clipBehavior: Clip.none, children: [
          IconButton(icon: const Icon(Icons.notifications_none, color: _navy, size: 28), onPressed: _showNotifications),
          if (_notificationCount > 0)
            Positioned(
              top: 6, right: 6,
              child: Container(width: 18, height: 18, decoration: const BoxDecoration(color: _red, shape: BoxShape.circle),
                child: Center(child: Text('$_notificationCount', style: const TextStyle(color: _white, fontSize: 10, fontWeight: FontWeight.w700))),
              ),
            ),
        ]),
        const SizedBox(width: 4),
...
class _SummaryCard extends StatelessWidget {
  ...
  const _SummaryCard({required this.label, required this.count, required this.color, required this.bgColor, required this.icon, required this.active, required this.onTap});
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: active ? color : const Color(0xFFE5E7EB), width: active ? 2 : 1),
            boxShadow: active ? [BoxShadow(color: color.withOpacity(0.15), blurRadius: 8, offset: const Offset(0, 2))] : [],
          ),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                const SizedBox(height: 8),
                const Text('Click to see details', style: TextStyle(fontSize: 11, color: Color(0xFF9CA3AF))),
              ]),
              Container(width: 48, height: 48, decoration: BoxDecoration(color: bgColor, shape: BoxShape.circle), child: Icon(icon, color: color, size: 24)),
            ],
          ),
        ),
      );
...
class _DetailPanel extends StatelessWidget {
  ...
  const _DetailPanel({required this.activeView, required this.available, required this.allInProgress, required this.validated, required this.provider, required this.superviseurId, required this.superviseurName});

  String get _title => switch (activeView) { 'received' => 'Manage Alerts Received', 'claimed' => 'Claimed Alerts', _ => 'Fixed Alerts' };

  @override
  Widget build(BuildContext context) => Container(
        width: double.infinity,
        decoration: BoxDecoration(color: _white, borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFE5E7EB))),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Padding(
            padding: const EdgeInsets.fromLTRB(16, 16, 16, 16),
            child: Row(children: [const Icon(Icons.info_outline, color: _navy, size: 20), const SizedBox(width: 8), Text(_title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: _navy))]),
          ),
          const Divider(height: 1, color: Color(0xFFE5E7EB)),
          const SizedBox(height: 16),
          if (activeView == 'received') _ReceivedView(alerts: available, provider: provider, superviseurId: superviseurId, superviseurName: superviseurName),
          if (activeView == 'claimed') _ClaimedView(alerts: allInProgress, provider: provider),
          if (activeView == 'fixed') _FixedView(alerts: validated, provider: provider),
          const SizedBox(height: 8),
        ]),
      );
}
...
class _ReceivedView extends StatelessWidget {
  ...
  final AlertProvider provider;
  final String superviseurId, superviseurName;
  const _ReceivedView({required this.alerts, required this.provider, required this.superviseurId, required this.superviseurName});

  @override
  Widget build(BuildContext context) {
    if (alerts.isEmpty) return _empty(Icons.notifications_off_outlined, Colors.orange, 'No alerts available', 'All alerts are being handled');
    return Column(children: alerts.map((a) => _AlertRow(
      alert: a,
      ...
      trailing: ElevatedButton.icon(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: _navy, foregroundColor: _white, padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8), textStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
      ),
    )).toList());
  }
}
...
class _ClaimedView extends StatelessWidget {
  ...
  void _resolveWithDialog(BuildContext context, AlertModel alert) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: TextField(controller: reasonController, decoration: const InputDecoration(hintText: 'Resolution reason'), maxLines: 3),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          ElevatedButton(
            onPressed: () async {
              if (reasonController.text.trim().isEmpty) return;
              await provider.resolveAlert(alert.id, reasonController.text.trim());
  ...
  Future<void> _getAiAssist(BuildContext context, AlertModel alert) async {
    ...
    final pastResolutions = await provider.getPastResolutionsForType(alert.type, 3);
    ...
    if (!alert.isCritical) {
      note = await showDialog<String>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: TextField(
            decoration: const InputDecoration(hintText: 'Optional note (reason, impact, etc.)'),
            maxLines: 3,
            onChanged: (value) => note = value,
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context, null), child: const Text('Cancel')),
            ElevatedButton(onPressed: () => Navigator.pop(context, note ?? ''), child: const Text('Mark Critical')),
          ],
        ),
      );
      if (note == null) return;
  ...
  Future<void> _offerAssistance(BuildContext context, AlertModel alert) async {
    await provider.requestHelp(alert.id, FirebaseAuth.instance.currentUser!.uid, FirebaseAuth.instance.currentUser!.email!.split('@').first);
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Assistance offered. The claimant will be notified.'), backgroundColor: Colors.purple));
  }

  String _getStatusLabel(AlertModel alert, bool isMine, String currentUserId) {
    final hasValidAssistant = alert.assistantId != null && alert.assistantId != alert.superviseurId;
    if (hasValidAssistant) {
      if (alert.assistantId == currentUserId) return 'Assisting ${alert.superviseurName}';
      else if (isMine) return 'My Claim (assisted by ${alert.assistantName ?? 'someone'})';
      else return 'Claimed by ${alert.superviseurName} (assisted by ${alert.assistantName ?? 'someone'})';
    }
  ...
  @override
  Widget build(BuildContext context) {
    if (alerts.isEmpty) return _empty(Icons.error_outline, const Color(0xFF94A3B8), 'No alerts in progress', 'Claim an alert to start');
        ...
        trailing: isMine
            ? Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  onPressed: () async {
                    ...
                    await showDialog(
                      ...
                      builder: (_) => AlertDialog(
                        ...
                        content: TextField(
                          decoration: const InputDecoration(hintText: 'Optional reason for suspension'),
                ...
                OutlinedButton.icon(
                  ...
                  label: const Text('AI Assist', style: TextStyle(fontSize: 11)),
                  style: OutlinedButton.styleFrom(
...
class _FixedView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    if (alerts.isEmpty) return _empty(Icons.check_circle_outline, Colors.green, 'No fixed alerts', 'Fixed alerts will appear here');
    return Column(children: alerts.map((a) => _AlertRow(
      alert: a,
      ...
      extraContent: Column(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFFDCFCE7), border: Border.all(color: const Color(0xFF86EFAC)), borderRadius: BorderRadius.circular(7)),
            child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.timer_outlined, size: 15, color: Color(0xFF16A34A)), const SizedBox(width: 6), Text('Resolution time: ${provider.formatElapsedTime(a.elapsedTime)}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF16A34A)))]),
          ),
          if (a.superviseurName != null)
            Padding(
              padding: const EdgeInsets.only(top: 6),
              child: Text.rich(TextSpan(children: [
                const TextSpan(text: 'Fixed by: ', style: TextStyle(fontSize: 12, color: Color(0xFF6B7280))),
                TextSpan(text: a.superviseurName, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _navy)),
                if (a.assistantName != null) TextSpan(text: ' (assisted by ${a.assistantName})', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w400, color: _muted)),
              ])),
            ),
        ],
      ),
...
class _AlertRow extends StatelessWidget {
  ...
  const _AlertRow({required this.alert, required this.rowColor, required this.statusLabel, required this.statusColor, this.borderColor, this.statusIcon, this.pulseDot = false, this.trailing, this.extraContent, this.onCriticalToggle, this.onRequestAssistance, this.onOfferAssistance});

  @override
  Widget build(BuildContext context) {
    final currentUserId = FirebaseAuth.instance.currentUser?.uid;
    final effectiveRowColor = alert.isCritical ? Colors.red.shade50 : rowColor;
    final effectiveBorderColor = alert.isCritical ? Colors.red.shade300 : (borderColor ?? const Color(0xFFE5E7EB));
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: effectiveRowColor, border: Border.all(color: effectiveBorderColor, width: borderColor != null ? 2 : 1), borderRadius: BorderRadius.circular(10)),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Row(children: [
                  if (alert.isCritical) const Icon(Icons.warning_amber_rounded, color: Colors.red, size: 16),
                  if (alert.isCritical) const SizedBox(width: 4),
                  pulseDot ? _PulseDot(color: _typeColor(alert.type)) : Container(width: 10, height: 10, decoration: BoxDecoration(color: _typeColor(alert.type), shape: BoxShape.circle)),
                  const SizedBox(width: 8),
                  Expanded(child: Text(_typeLabel(alert.type), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _navy))),
                  if (alert.status == 'en_cours' && alert.superviseurId == currentUserId && onCriticalToggle != null) IconButton(onPressed: onCriticalToggle, icon: Icon(alert.isCritical ? Icons.warning_rounded : Icons.warning_amber_outlined, color: alert.isCritical ? Colors.red : Colors.orange, size: 20), tooltip: alert.isCritical ? 'Remove critical flag' : 'Mark as critical', padding: EdgeInsets.zero, constraints: const BoxConstraints()),
                ]),
                const SizedBox(height: 6),
                Wrap(spacing: 6, runSpacing: 4, children: [
                  _OutlineBadge('${alert.usine} — Line ${alert.convoyeur} — Workstation ${alert.poste}'),
                  _FilledBadge(label: statusLabel, color: statusColor, icon: statusIcon),
                ]),
                const SizedBox(height: 6),
                Text(alert.description, style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280))),
                const SizedBox(height: 4),
...
class _ElapsedTimer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      margin: const EdgeInsets.only(top: 8),
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
      decoration: BoxDecoration(color: const Color(0xFFDBEAFE), border: Border.all(color: const Color(0xFF93C5FD)), borderRadius: BorderRadius.circular(7)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        const Icon(Icons.timer_outlined, size: 15, color: Color(0xFF1D4ED8)),
        const SizedBox(width: 6),
        Flexible(child: Text('Elapsed time: $elapsed', style: const TextStyle(fontFamily: 'monospace', fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF1D4ED8)), overflow: TextOverflow.ellipsis)),
      ]),
    );
  }
}
...
class _PulseDotState extends State<_PulseDot> with SingleTickerProviderStateMixin {
  ...
  @override void initState() { super.initState(); _ctrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 900))..repeat(reverse: true); _anim = Tween(begin: 0.3, end: 1.0).animate(_ctrl); }
  @override void dispose() { _ctrl.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) => FadeTransition(opacity: _anim, child: Container(width: 10, height: 10, decoration: BoxDecoration(color: widget.color, shape: BoxShape.circle)));
}

class _OutlineBadge extends StatelessWidget {
  final String text;
  const _OutlineBadge(this.text);
  @override Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(border: Border.all(color: const Color(0xFFD1D5DB)), borderRadius: BorderRadius.circular(99)), child: Text(text, style: const TextStyle(fontSize: 10, color: Color(0xFF374151))));
}
...
class _FilledBadge extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      if (icon != null) ...[Icon(icon, size: 11, color: _white), const SizedBox(width: 3)],
      Flexible(child: Text(label, style: const TextStyle(fontSize: 10, color: _white, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis, maxLines: 1)),
    ]),
  );
}
...
Widget _empty(IconData icon, Color color, String title, String sub) => Padding(
      ...
      child: Center(
        child: Column(children: [
          Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: color.withOpacity(0.1), shape: BoxShape.circle), child: Icon(icon, size: 36, color: color)),
          const SizedBox(height: 12),
          Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
          const SizedBox(height: 4),
          Text(sub, style: const TextStyle(fontSize: 13, color: Color(0xFF9CA3AF))),
        ]),
      ),
    );