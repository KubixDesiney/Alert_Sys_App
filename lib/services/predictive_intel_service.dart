...
/// Bridges the Cloudflare Worker predictive engine + Firebase cache.
///
/// All endpoints fall back to in-memory defaults so the UI never crashes
/// when offline or while predictions are still warming up.
class PredictiveIntelService {
  ...
  /// Streams the cached briefing from Firebase. Emits null if missing.
  Stream<MorningBriefing?> briefingStream() {
    return _db.ref('ai_briefing/latest').onValue.map((event) {
      final v = event.snapshot.value;
      if (v is! Map) return null;
      return MorningBriefing.fromMap(Map<String, dynamic>.from(v));
    });
  }

  Future<MorningBriefing?> fetchBriefing({bool force = false}) async {
    try {
      final res = await http
          .get(Uri.parse('$_workerBase/briefing${force ? '?force=1' : ''}'))
          .timeout(_timeout);
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        if (data is Map) {
          return MorningBriefing.fromMap(Map<String, dynamic>.from(data));
        }
      }
    } catch (_) {}
    return null;
  ...
  // ── Predictions (curves + machine forecast) ─────────────────────────────

  Stream<PredictiveModel?> predictionsStream() {
    return _db.ref('ai_predictions/latest').onValue.map((event) {
      final v = event.snapshot.value;
      if (v is! Map) return null;
      return PredictiveModel.fromMap(Map<String, dynamic>.from(v));
    });
  }

  Future<PredictiveModel?> fetchPredictions() async {
    try {
      final res =
          await http.get(Uri.parse('$_workerBase/predict')).timeout(_timeout);
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        if (data is Map) {
          return PredictiveModel.fromMap(Map<String, dynamic>.from(data));
        }
      }
    } catch (_) {}
    return null;
  ...
  // ── One-tap assignee suggestion ─────────────────────────────────────────

  Future<AssigneeSuggestion?> suggestAssignee(String alertId) async {
    try {
      final res = await http
          .get(Uri.parse('$_workerBase/suggest-assignee?alertId=$alertId'))
          .timeout(_timeout);
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body);
        if (data is Map) {
          return AssigneeSuggestion.fromMap(Map<String, dynamic>.from(data));
        }
      }
    } catch (_) {}
    return null;
    ...
    return PredictiveModel(
      ...
      predictions: preds is List
          ? preds
              .whereType<Map>()
              .map((e) =>
                  PredictedFailure.fromMap(Map<String, dynamic>.from(e)))
              .toList()
          : const [],
